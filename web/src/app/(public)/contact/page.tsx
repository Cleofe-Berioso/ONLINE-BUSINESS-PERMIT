  ...
  return (
    <div className="min-h-screen bg-[var(--surface-muted)] flex flex-col">
      ...
      <main className="flex-1 mx-auto w-full max-w-4xl px-4 py-8 sm:py-12">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-[var(--text-primary)] mb-2">Contact Us</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          {contactInfo.map((info) => (
            <div key={info.title} className="bg-[var(--surface)] rounded-xl shadow-sm border p-5">
              <div className={`w-11 h-11 ${info.bg} rounded-lg flex items-center justify-center mb-3`}>
                {info.icon}
              </div>
              <h3 className="font-semibold text-[var(--text-primary)] mb-2 text-sm sm:text-base">{info.title}</h3>
        ...
        {/* Contact Form */}
        <div className="bg-[var(--surface)] rounded-xl shadow-sm border p-5 sm:p-6">
          <h2 className="text-lg sm:text-xl font-semibold text-[var(--text-primary)] mb-5">Send Us a Message</h2>
          <ContactForm />
        </div>
      </main>
